...
export const BookGrid = ({ books, loading, onBookClick }) => {
    if (loading) return <div className="grid grid-cols-2 md:grid-cols-3 gap-4">{[...Array(6)].map((_, idx) => <div key={idx} className="animate-pulse bg-gray-200 h-48 w-full rounded-xl" />)}</div>;
    ...
    return (
        <div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {books.map((book, idx) => <BookCard key={book.id} book={book} onClick={() => onBookClick(book)} className="fade-in-up" style={{ animationDelay: `${idx * 0.1}s` }} />)}
            </div>
        </div>
    );
};
